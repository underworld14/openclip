...
export function registerModelHandlers(ctx: IpcContext): void {
  ...
  //
  // Reports the truth in every mode. An earlier revision special-cased
  // OPENCLIP_FAKE_TRANSCRIBE to report "installed", but that was only needed
  // because `canGenerate` wrongly required a transcriber to be present in order
  // to analyse an EXISTING transcript. Decoupling those in `readinessView`
  // removed the need for the workaround — one fewer place where an env var can
  // make the app misreport a capability.
  ctx.ipcMain.handle(
